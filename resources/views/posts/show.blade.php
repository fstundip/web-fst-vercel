...
@section('container')
<div>
    <h2 class="text-center text-white fw-semibold py-4 bg-succes">{{ $post->title }}</h2>
    ...
    <div class="card-blog row mb-4">
            ...
            <div class="card border-0 h-100 text-decoration-none">
                <div class="card-body">
                    <div id="img-news" style="display: block;">
                        <img src="{{ asset('storage/' . $post->image)}}" alt="" class="rounded mb-4"
                            style="width: 100%;">
                    </div>
                    <span class="card-title fw-bold" style="font-size: 24px; color:#687281">{{ $post->title }}</span>
                    <p class="card-text"><small class="text-muted">{{$post->created_at->format('l, F j, Y')}}, by
                            Admin</small></p>
                    <p style="line-height: 1.75; color:#687281">{!! $post->body !!}</p>
                </div>
            </div>
        </div>
        <div class="card-other col-xl-4 mt-4 mt-xl-0">
            <div class="card-content">
                ...
                <iframe width="100%" height="100%" src="https://www.youtube.com/embed/8RQqY6LMc_o?si=wjWw5jlumcJH3orZ"
